...
) =>
{
	...
	chai.Assertion.addMethod(
		...
		function(event, options = {})
		{
			...
			options.argsMatch = options.argsMatch ?? emitDefaults.argsMatch ?? 'soft';
			options.count = options.count ?? emitDefaults.count ?? { min: 1, max: 1 };
			options.orderMatters = options.orderMatters ?? emitDefaults.orderMatters ?? false;
			options.withArgs = options.withArgs ?? emitDefaults.withArgs ?? null;
	...
	chai.Assertion.addMethod('on', function(testingBlock)
	{
			...
			.forEach(([event, expectations]) =>
			{
				const handler = function(...args)
				{
					...
					else
					{
						...
						try
						{
							argsMessage = `\nArguments: ${JSON.stringify(args)}.\nExpectations: ${expectations.map(({ options }) => options).map(JSON.stringify).join('\n')}`;
						} catch(e) {}
		...
		// Validates errors
		this.assert(!errors.length, errors[0]);
